...
export default function UserPage() {
    ...
    return (
        <div className="container mx-auto px-3 sm:px-4 md:px-6 py-4 flex flex-col lg:flex-row gap-3 sm:gap-4 md:gap-6">
            {/* 左侧导航：移动端横向滚动，桌面端竖栏 */}
            <div className="flex lg:flex-col gap-2 lg:w-64 bg-background rounded-lg p-3 lg:p-4 shadow-md overflow-x-auto">
                {/* 菜单列表 */}
                <div className="flex lg:flex-col gap-2">
                    {menuList.map((item) => (
                        <div
                            key={item.value}
                            className={`h-10 px-4 lg:px-0 lg:w-full text-sm sm:text-base whitespace-nowrap flex-shrink-0 hover:bg-blue-100 rounded-md transition-transform duration-300 cursor-pointer text-center items-center justify-center flex active:scale-95 ${
                                active === item.index
